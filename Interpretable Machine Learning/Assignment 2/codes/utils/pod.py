...
def compute_reduced_coefficients(u, V):
    """Project a full-order snapshot onto the POD basis to get the POD coefficients
    
    Arguments:
        u {np.ndarray} -- Full-order snapshot matrix, of shape [N_sample, N_feature]
...
def compute_reduced_approximation(c, V):
    """Compute the reduced approximation by a linear combination of POD basis
    ...
    Returns:
        u -- Reduced approximation, of shape [N_sample, N_feature]
    """
    return c.dot(V)