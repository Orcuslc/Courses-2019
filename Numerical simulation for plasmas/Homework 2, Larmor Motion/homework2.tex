...
\item
Figure \ref{problem 2b.2} is the error plot with respect to the number of timesteps. The slope is $k = -2$. (I wonder if the abnormal points are caused by the instability since I first use Euler forward scheme to get $x_1 $ and $v_1 $ with given $x_0 $ and $v_0 $.)
\begin{figure}[h]
\centering
\vbox{
